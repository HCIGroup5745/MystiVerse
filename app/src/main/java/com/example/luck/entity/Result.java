package com.example.luck.entity;

public class Result {

    private String icon;
    private String name;
    private String range;
    private String zxtd;
    private String sssx;
    private String zggw;
    private String yysx;
    private String zdtz;
    private String zgxx;
    private String xyys;
    private String jssw;
    private String xyhm;
    private String kyjs;
    private String bx;
    private String yd;
    private String qd;
    private String jbtz;
    private String jttz;
    private String xsfg;
    private String gxmd;
    private String zj;
    public void setIcon(String icon) {
        this.icon = icon;
    }
    public String getIcon() {
        return icon;
    }

    public void setName(String name) {
        this.name = name;
    }
    public String getName() {
        return name;
    }

    public void setRange(String range) {
        this.range = range;
    }
    public String getRange() {
        return range;
    }

    public void setZxtd(String zxtd) {
        this.zxtd = zxtd;
    }
    public String getZxtd() {
        return zxtd;
    }

    public void setSssx(String sssx) {
        this.sssx = sssx;
    }
    public String getSssx() {
        return sssx;
    }

    public void setZggw(String zggw) {
        this.zggw = zggw;
    }
    public String getZggw() {
        return zggw;
    }

    public void setYysx(String yysx) {
        this.yysx = yysx;
    }
    public String getYysx() {
        return yysx;
    }

    public void setZdtz(String zdtz) {
        this.zdtz = zdtz;
    }
    public String getZdtz() {
        return zdtz;
    }

    public void setZgxx(String zgxx) {
        this.zgxx = zgxx;
    }
    public String getZgxx() {
        return zgxx;
    }

    public void setXyys(String xyys) {
        this.xyys = xyys;
    }
    public String getXyys() {
        return xyys;
    }

    public void setJssw(String jssw) {
        this.jssw = jssw;
    }
    public String getJssw() {
        return jssw;
    }

    public void setXyhm(String xyhm) {
        this.xyhm = xyhm;
    }
    public String getXyhm() {
        return xyhm;
    }

    public void setKyjs(String kyjs) {
        this.kyjs = kyjs;
    }
    public String getKyjs() {
        return kyjs;
    }

    public void setBx(String bx) {
        this.bx = bx;
    }
    public String getBx() {
        return bx;
    }

    public void setYd(String yd) {
        this.yd = yd;
    }
    public String getYd() {
        return yd;
    }

    public void setQd(String qd) {
        this.qd = qd;
    }
    public String getQd() {
        return qd;
    }

    public void setJbtz(String jbtz) {
        this.jbtz = jbtz;
    }
    public String getJbtz() {
        return jbtz;
    }

    public void setJttz(String jttz) {
        this.jttz = jttz;
    }
    public String getJttz() {
        return jttz;
    }

    public void setXsfg(String xsfg) {
        this.xsfg = xsfg;
    }
    public String getXsfg() {
        return xsfg;
    }

    public void setGxmd(String gxmd) {
        this.gxmd = gxmd;
    }
    public String getGxmd() {
        return gxmd;
    }

    public void setZj(String zj) {
        this.zj = zj;
    }
    public String getZj() {
        return zj;
    }

    @Override
    public String toString() {
        return "Result{" +
                "icon='" + icon + '\'' +
                ", name='" + name + '\'' +
                ", range='" + range + '\'' +
                ", zxtd='" + zxtd + '\'' +
                ", sssx='" + sssx + '\'' +
                ", zggw='" + zggw + '\'' +
                ", yysx='" + yysx + '\'' +
                ", zdtz='" + zdtz + '\'' +
                ", zgxx='" + zgxx + '\'' +
                ", xyys='" + xyys + '\'' +
                ", jssw='" + jssw + '\'' +
                ", xyhm='" + xyhm + '\'' +
                ", kyjs='" + kyjs + '\'' +
                ", bx='" + bx + '\'' +
                ", yd='" + yd + '\'' +
                ", qd='" + qd + '\'' +
                ", jbtz='" + jbtz + '\'' +
                ", jttz='" + jttz + '\'' +
                ", xsfg='" + xsfg + '\'' +
                ", gxmd='" + gxmd + '\'' +
                ", zj='" + zj + '\'' +
                '}';
    }
}
